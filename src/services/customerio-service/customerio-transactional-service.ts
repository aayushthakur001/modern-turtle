...
export interface OrganizationMembershipInviteEmailParams {
...
export async function organizationMembershipInviteEmail(email: string, data: OrganizationMembershipInviteEmailParams): Promise<void> {
    await genericSendAnonymousEmail(config.customer.transactionalMessages.organizationMembershipInvite, email, data);
}
...
export async function sendPlansEmail(project: IProjectDoc, tierId: string, event: PlansEmailEvent) {
    ...
    const transactionalMessageId = (
        config.customer.transactionalMessages.plans as Record<string, Partial<Record<PlansEmailEvent, number>>>
    )[tierId]?.[event];
